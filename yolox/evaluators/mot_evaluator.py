...
from trackers.SUT_UKF import SU_T
from trackers.SUT_reid_UKF import SU_T_ReID
import contextlib
import io
import os
import itertools
...
class MOTEvaluator:
    ...
    def evaluate_su_t(
        ...
        for cur_iter, (imgs_, _, info_imgs, ids, raw_image) in enumerate(    # [hgx0411] add raw_image for FastReID
            ...
            with torch.no_grad():
                ...
                if os.path.exists(ckt_file):
                    ...
                    outputs_raw = [data['detection']]
                else:
                    imgs = imgs_.type(tensor_type)

                    # skip the the last iters since batchsize might be not enough for batch inference
                    outputs_raw = model(imgs)
                    if decoder is not None:
                        outputs_raw = decoder(outputs_raw, dtype=outputs_raw.type())

                    outputs = postprocess(outputs_raw, self.num_classes, self.confthre, self.nmsthre)
    ...
    def evaluate_su_t_reid(
        ...
        for cur_iter, (imgs, _, info_imgs, ids, raw_image) in enumerate(    # [hgx0411] add raw_image for FastReID
            ...
            with torch.no_grad():
                ...
                if frame_id == 1:
                    tracker = SU_T_ReID(args, det_thresh=self.args.track_thresh, iou_threshold=self.args.iou_thresh,
                                     asso_func=self.args.asso, delta_t=self.args.deltat, inertia=self.args.inertia)
                    if len(results) != 0:
                        result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[video_id - 1]))
                        write_results_no_score(result_filename, results)
